...
app.post('/api/contact', async (req, res) => {
  try {
    ...
    console.log('Contact form received:', { name, email, phone, message, program, timestamp: new Date().toISOString() });
...
Please let me know the next steps. Thank you!`;
    ...
    return res.status(200).json({ success: true, redirectUrl: whatsappUrl, message: 'Redirecting to WhatsApp...' });
  } catch (error) {
...
app.post('/api/phone-inquiry', async (req, res) => {
  try {
    ...
    console.log('Phone inquiry:', { visitorPhone, inquiryType, timestamp: new Date().toISOString() });
    return res.status(200).json({ success: true, message: 'Please call +91 81213 34894 to speak with our team.' });
  } catch (error) {
...
app.post('/api/whatsapp', async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, url: whatsappUrl });
  } catch (error) {
